...
func (h *HysteriaOption) Build() any {
	...
	h.TFO = h.FastOpen
	serverOptions := h.ServerOptions.Build()
	if serverOptions.ServerPort == 0 {
		serverOptions.ServerPort = clashFirstPort(h.Ports)
	}
...
func clashFirstPort(ports string) uint16 {
	if ports == "" {
		return 0
	}
	ports = strings.ReplaceAll(ports, "/", ",")
	first := strings.TrimSpace(strings.Split(ports, ",")[0])
	if separator := strings.IndexAny(first, "-:"); separator >= 0 {
...
func clashTLSOptions(server string, tlsOptions *TLSOptions) option.OutboundTLSOptionsContainer {
	// A Clash proxy with an explicit `tls: false` must not create a disabled
	// TLS object. Several sing-box outbounds use a non-nil TLS object as the
	// signal that TLS is configured.
	if tlsOptions == nil || !tlsOptions.TLS {
		return option.OutboundTLSOptionsContainer{}
	}
	if tlsOptions.SNI == "" {
		tlsOptions.SNI = server
	}
	return option.OutboundTLSOptionsContainer{TLS: tlsOptions.Build()}
}
